...
- `--no-resize` buys the selected VM directly (at least one prepaid day) while preserving the shorter task deadline. Use it only when direct provisioning is intentionally required; preview and fallback candidates must all remain direct.
