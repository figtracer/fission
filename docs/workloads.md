...
Use recipe `tempo` for the isolated development node on guest loopback port 8645. Supply a signer separately and check `node` before execution. Prebuilt Cast 1.8.1 worked for both ordinary EIP-1559 and native Tempo type `0x76` transactions on Tempo 1.14.0; no SDK dependency or client source build was needed. Compose prebuilt Foundry preparation when Cast is required; the `tempo` recipe itself installs only the node.
...
September 15 native validation included **seven `0x76` transactions: six successes and one deliberate batch revert**. Transfer, deployment, two-call execution, sponsorship, and nonce lanes 7/11 passed receipt, inclusion, calldata, state and fee checks. A batch writing 3 then reverting on 7 preserved the earlier value 2, while consuming its nonce and charging 47 AlphaUSD base units. The sponsored write charged the separate sponsor 42 units and left the sender's fee-token balance unchanged. All seven cost 3,152 units: 3,110 from the sender and 42 from the sponsor. Both nonzero lanes accepted nonce 0 and advanced to 1 while the ordinary nonce stayed 5; submission was sequential, not a throughput/concurrency benchmark. Stale-lane, wrong-chain, native-value and unfunded-sponsor transactions were rejected by RPC without changing the observed nonces, balances or storage.

Use `cast mktx --tempo.fee-token TOKEN` for an explicitly selected native envelope and `cast batch-mktx --call "TO:0:0xCALLDATA" --call "TO:0:0xCALLDATA"` for a batch, with explicit chain, gas/fee limits and signer options. Record the signed raw payload before submitting it once. Require raw prefix and returned transaction/receipt type `0x76`; an estimation failure is not an included-revert test. Sponsorship used `--tempo.sponsor ADDRESS --tempo.sponsor-signer env://VARIABLE`, keeping the public development key out of persisted argv. For lanes, supply **both `--tempo.nonce-key KEY` and `--nonce N`**; read nonzero keys through `getNonce(address,uint256)` at `0x4e4f4e4345000000000000000000000000000000`, and key 0 through `eth_getTransactionCount`. Do not assume ordinary nonce autofill selects the lane. Expiring nonces, access keys, remote sponsorship services, restart/persistence and reorgs remain untested. References: [pinned Cast Tempo options](https://github.com/foundry-rs/foundry/blob/982849d3140c01fd3b72905759581a132df7aa98/crates/cli/src/opts/tempo.rs), [batch signing](https://github.com/foundry-rs/foundry/blob/982849d3140c01fd3b72905759581a132df7aa98/crates/cast/src/cmd/batch_mktx.rs).
...
**Fund isolated calls for fees, not just transferred value.** This binary defaults to `isolate = true`. The earlier exactly-1-ETH-funded WETH deposit reverted before contract execution; a deliberately non-isolated roundtrip passed. September 15 validation at the same block, with Solidity 0.8.30, `network = "optimism"`, `hardfork = "optimism:isthmus"`, and `isolate = true`, measured **3,144,402,259 wei per isolated call** credited to the L1 fee vault (`0x420000000000000000000000000000000000001A`). The caller's balance loss matched value plus that fee. Operator scalar/constant and operator/base-fee/coinbase deltas were zero for the measured deposit.

The independent Fjord-floor calculation `100,000,000 × (16 × baseFeeScalar × l1BaseFee + blobBaseFeeScalar × blobBaseFee) / 10^12`, rounded down, matched both the observed fee and GasPriceOracle `getL1Fee(hex"00")`. Parameters were 2,269 / 310,859,270 / 1,055,762 / 19,093,875 respectively. Funding the 1 ETH deposit with one wei less than this fee rejected it without state changes; funding the exact fee succeeded and left zero balance. With fee headroom, the isolated 1 ETH deposit / 0.4 WETH transfer / 0.6 ETH withdrawal passed token/reserve checks and charged **9,433,206,777 wei** across three calls. Disabling isolation made the rejection and fee-accounting assertions fail, as intended. Forge normalizes the configured hardfork to `optimism:Isthmus` in JSON; inspect resolved config rather than comparing its capitalization to TOML input.

These are **Forge synthetic-transaction fees, not a quote for a signed Base transaction**. The pinned isolated-call executor zeroes inner gas price/base fee, and this observation exercises the L1-fee floor, not complete L2 execution-price, operator-fee or serialized-transaction-size behavior. Recompute for the chosen block and settings; do not hard-code the measured fee into another workload. Use `--no-isolate` only for deliberately selected contract-call tests, not to bypass fee failures, and never switch to Ethereum just to obtain a pass.
