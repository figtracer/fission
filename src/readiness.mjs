...
export async function check(name, scope, seconds, maxHeadAge) {
  ...
  return locked(name, async () => {
    ...
    let checks = state.recipe.checks?.filter((probe) => probe.scope === scope) || [];
    if (!checks.length && scope === "tools") checks = (state.recipe.readiness || []).map((argv, i) => ({ name: `tool-${i + 1}`, scope, argv, result: "exit" }));
    if (!checks.length && scope === "build" && Object.hasOwn(sourceRecipes, state.recipe.name))
      checks = [{ name: "compiled-artifacts", scope, argv: ["python3", "/workspace/.fission/rust-source.py", "verify", ...sourceRecipes[state.recipe.name]], result: "json" }];
    if (!checks.length && scope === "node" && state.recipe.name === "reth-synced") {
